...
function replaceGraph<
  ...
  TSelection,
>(
  current: GraphSnapshot<TNode, TEdge, TSelection>,
  payload: ReplaceGraphPayload<TNode, TEdge, TSelection>,
): GraphSnapshot<TNode, TEdge, TSelection> {
  return {
    nodes: payload.nodes,
    edges: payload.edges,
...
): ExecutedGraphCommand<TNode, TEdge, TSelection> & {
  next: GraphSnapshot<TNode, TEdge, TSelection>;
} {
  switch (command.kind) {
    case "add_node": {
    ...
    case "update_node": {
      ...
      const next = {
        ...
        nodes: current.nodes.map((item) =>
          item.id === command.payload.nodeId ? command.payload.nextNode : item,
        ),
      };
      return {
        command,
    ...
    case "update_edge": {
      ...
      const next = {
        ...
        edges: current.edges.map((item) =>
          item.id === command.payload.edgeId ? command.payload.nextEdge : item,
        ),
      };
      return {
        command,
