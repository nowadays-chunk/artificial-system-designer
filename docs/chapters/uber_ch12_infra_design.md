# Uber Case Study - Chapter 12: Collaboration and Stakeholder Communication
...
- **Problem**: Users received HTTP 500 errors when requesting rides or viewing active map overlays.
- **Why?**: The Dispatch Matcher Service could not fetch cached driver locations from Redis.
- **Why?**: The Redis cluster node became unresponsive due to memory saturation.
- **Why?**: A high-demand zone with 10,000 active riders updated their coordinates, triggering a write-intensive fan-out storm that saturated the cache node.
- **Why?**: The hybrid pull-on-read model threshold was set to 50,000 users, which was too high to prevent write storms on hot zones.
- **Remediation**: Adjust the active rider write-bypass threshold to 100 users and enforce rate limits on fan-out queues.
