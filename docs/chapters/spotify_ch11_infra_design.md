...
```
       [ Delete User Request ] ---> [ Soft Delete (Metadata flag) ]
         ...
         |                                                                             |
         v                                                                             v
  [ Delete SQL Rows (Profiles, Likes) ]                                 [ Delete Cassandra Rows (Playlists) ]
```

- **Right to Be Forgotten**: When a user deletes their profile, the delete service deletes their record from PostgreSQL and publishes a deletion event to a Kafka topic. Background jobs consume the event to delete the user's playlist records and history logs from Cassandra, avoiding database write saturation during peak hours.
- **Data Export**: Background workers package user data, playlist logs, and history streams into encrypted ZIP archives, providing download links to users requesting data exports.
