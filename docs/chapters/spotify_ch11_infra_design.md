# Spotify Case Study - Chapter 11: Governance and Compliance
...
3. **Asynchronous Hard Delete**: A background job processes the deletion log from a Kafka topic (`account-deletions`). It runs low-priority batch deletions against databases to remove all playlists, follows, and likes associated with the user ID, avoiding database write saturation during peak hours.
...
export class DeletionPipeline {
  ...
  private pgClient = new PostgresClient({
    connectionString: "postgresql://prod-db:5432/spotify"
  });
  ...
  private async executeHardDelete(userId: string) {
    ...
    // 1. Delete user's playlists from PostgreSQL
    const deletePlaylistsQuery = "DELETE FROM spotify_metadata.playlists WHERE owner_id = $1";
    await this.pgClient.query(deletePlaylistsQuery, [userId]);

    // 2. Delete user's social graph relations
    const deleteFollowersQuery = "DELETE FROM spotify_metadata.follows WHERE follower_id = $1 OR followed_id = $1";
...
2. **Asynchronous Assembly**: A worker consumes the request and queries database systems (PostgreSQL) to retrieve the user's profile, playlists lists, and historical listening logs.
...
metadata:
  name: playback-service-pod
  labels:
    env: production
    owner: core-playback-team
    project: spotify-platform
    cost-center: cc-2048
