# WhatsApp Case Study - Chapter 12: Collaboration and Stakeholder Communication
...
- **Problem**: Users received HTTP 500 errors when routing messages or loading active contacts presence.
- **Why?**: The Message Router Service could not fetch cached presence logs from Redis.
- **Why?**: The Redis cluster node became unresponsive due to memory saturation.
- **Why?**: A public business channel with 10,000 active subscribers posted a message, triggering a write-intensive fan-out storm that saturated the cache node.
- **Why?**: The hybrid pull-on-read model threshold was set to 50,000 subscribers, which was too high to prevent write storms on active channels.
- **Remediation**: Adjust the channel subscriber write-bypass threshold to 500 subscribers and enforce rate limits on fan-out queues.
