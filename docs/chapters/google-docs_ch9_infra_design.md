...
```
       [ EKS Kubernetes Worker Node Array ]
         ...
         +-------------+-------------+
         v                           v
  [ Pod: Document Reader ]    [ Pod: WebSocket Sync ]
...
kind: Deployment
metadata:
  name: reader-service-deployment
  namespace: googledocs-prod
spec:
  replicas: 48
  selector:
    matchLabels:
  ...
  template:
    ...
    spec:
      containers:
      - name: reader-service-container
        image: googledocs/reader-service:v3.2.0
        resources:
          requests:
            memory: "4Gi"
...
```
This configuration forces EKS to distribute `reader-service` pods across distinct availability zones, ensuring that an AZ outage does not disrupt service availability.
...
For CPU-bound compute services (e.g., Document Reader API), the scale target is set to **65% average CPU utilization**.
