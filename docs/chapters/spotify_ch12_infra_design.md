# Spotify Case Study - Chapter 12: Collaboration and Stakeholder Communication
...
- **Problem**: Users received HTTP 500 errors when loading home feeds or playlists.
- **Why?**: The Playback Service could not fetch cached track lists from Redis.
- **Why?**: The Redis cluster node became unresponsive due to memory saturation.
- **Why?**: A celebrity artist with 10 million followers uploaded a track, triggering a write-intensive fan-out storm that saturated the cache node.
- **Why?**: The hybrid pull-on-read model threshold was set to 50,000 followers, which was too high to prevent write storms on celebrity tracks.
