...
3. **Asynchronous Hard Delete**: A background job processes the deletion log from a Kafka topic (`account-deletions`). It runs low-priority batch deletions against databases to remove all documents, comments, and shared folder permissions associated with the user ID, avoiding database write saturation during peak hours.
...
export class DeletionPipeline {
  ...
  private pgClient = new PostgresClient({
    connectionString: "postgresql://prod-db:5432/googledocs"
  });
  ...
  private async executeHardDelete(userId: string) {
    ...
    // 1. Delete user's files and document metadata from PostgreSQL
    const deleteDocsQuery = "DELETE FROM google_docs_metadata.documents WHERE owner_id = $1";
    await this.pgClient.query(deleteDocsQuery, [userId]);

    // 2. Delete user's shared document memberships
    const deleteMembershipsQuery = "DELETE FROM google_docs_metadata.collaborators WHERE user_id = $1";
...
2. **Asynchronous Assembly**: A worker consumes the request and queries database systems (PostgreSQL) to retrieve the user's profile, documents list, and sharing histories.
3. **Packaging**: The worker compiles the dataset into a structured JSON payload, packages it into an encrypted ZIP file containing their converted document formats (e.g. PDF/DOCX), and uploads it to a secure S3 bucket.
...
metadata:
  name: reader-service-pod
  labels:
    env: production
    owner: core-reader-team
    project: googledocs-platform
    cost-center: cc-4096
