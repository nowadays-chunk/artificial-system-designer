# Dropbox Case Study - Chapter 10: Disaster Recovery and Business Continuity
...
```
         ...
         |  Block Ingestion  |  < 2 minutes       |  < 1 second      |
         +-----------------------------------------------------------+
```
...
### 2.1 Block Storage Replication (Cross-Region Replication)
For unique chunk binary blocks, we use multi-region storage replication:
- **Asynchronous Replication**: Object binaries are copied asynchronously to the target region.
- **RPO Target**: Replication completes in under 15 minutes for 99.9% of blocks.

---

### 2.2 PostgreSQL Active-Passive Replication (Strict Consistency)
For transactional file metadata and namespace configurations, active-active setups can cause write conflicts. We deploy an **Active-Passive replication** model:
...
export class DNSFailoverController {
  private primaryRegionHealthUrl = "https://health.us-east.dropbox.com";
  private standbyRegionHealthUrl = "https://health.us-west.dropbox.com";
