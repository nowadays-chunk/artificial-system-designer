# Uber Case Study - Chapter 11: Governance and Compliance
...
Global dispatch platforms must comply with data privacy regulations like **GDPR (General Data Protection Regulation)** and **CCPA (California Consumer Privacy Act)**. Key compliance requirements include:
...
2. **Cache Eviction**: The system evicts the user's active session and location history caches from Redis instantly.
3. **Asynchronous Hard Delete**: A background job processes the deletion log from a Kafka topic (`account-deletions`). It runs low-priority batch deletions against databases to remove all trips, driver records, and payment logs associated with the user ID, avoiding database write saturation during peak hours.
...
export class DeletionPipeline {
  ...
  private pgClient = new PostgresClient({
    connectionString: "postgresql://prod-db:5432/uber"
  });
  ...
  private async executeHardDelete(userId: string) {
    ...
    // 1. Delete user's trips history from PostgreSQL
    const deleteTripsQuery = "DELETE FROM uber_metadata.trips WHERE rider_id = $1 OR driver_id = $1";
    await this.pgClient.query(deleteTripsQuery, [userId]);

    // 2. Delete user's rating records
    const deleteRatingsQuery = "DELETE FROM uber_metadata.ratings WHERE rater_id = $1 OR ratee_id = $1";
    await this.pgClient.query(deleteRatingsQuery, [userId]);
...
2. **Asynchronous Assembly**: A worker consumes the request and queries database systems (PostgreSQL) to retrieve the user's profile, trip logs, and invoice receipts.
3. **Packaging**: The worker compiles the dataset into a structured JSON payload, packages it into an encrypted ZIP file containing their invoices, and uploads it to a secure S3 bucket.
...
metadata:
  ...
  labels:
    env: production
    owner: core-dispatch-team
    project: uber-platform
    cost-center: cc-9216
