# Uber Case Study - Chapter 10: Disaster Recovery and Business Continuity
...
```
         ...
         |  Namespace Read   |  < 10 minutes      |  < 5 minutes     |
         |  Dispatch Ingest  |  < 2 minutes       |  < 1 second      |
         +-----------------------------------------------------------+
```
...
### 2.1 Map Data Storage Replication (Cross-Region Replication)
For base maps tiles and regional configurations, we use multi-region storage replication:
- **Asynchronous Replication**: Object binaries are copied asynchronously to the target region.
- **RPO Target**: Replication completes in under 15 minutes for 99.9% of updates.

---

### 2.2 PostgreSQL Active-Passive Replication (Strict Consistency)
For transactional user billing profiles and system configurations, active-active setups can cause write conflicts. We deploy an **Active-Passive replication** model:
...
export class DNSFailoverController {
  private primaryRegionHealthUrl = "https://health.us-east.uber.com";
  private standbyRegionHealthUrl = "https://health.us-west.uber.com";
