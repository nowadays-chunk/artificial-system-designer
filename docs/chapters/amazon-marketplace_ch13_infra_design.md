# Amazon Marketplace Case Study - Chapter 13: Infrastructure Optimization
...
- **Cache Hit Ratio**: Active catalog and inventory cache hits must exceed 98%. Drops in this ratio shift read loads to the PostgreSQL database, increasing read latency.
...
```
  ...
  [ Uncompressed Video ]     =======( Brotli Catalog Compression )=====> [ Stream Feeds (60% smaller) ]
```
...
### 2.2 Catalog Data Compression
Catalog and product details payloads are processed through compression pipelines:
- Product lists are compressed using **Brotli** or **ZSTD** algorithms before transmission, reducing network bandwidth footprints.
...
For Java-based Checkout Services, we use the **Z Garbage Collector (ZGC)**:
