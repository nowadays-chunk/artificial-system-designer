...
```
       [ EKS Kubernetes Worker Node Array ]
         ...
         +-------------+-------------+
         v                           v
  [ Pod: Playback Service ]  [ Pod: Audio Ingestion ]
  ...
  template:
    ...
    spec:
      ...
      - name: playback-service-container
        image: spotify/playback-service:v3.2.0
        resources:
          requests:
            memory: "4Gi"
...
```
This configuration forces EKS to distribute `playback-service` pods across distinct availability zones, ensuring that an AZ outage does not disrupt service availability.
...
For CPU-bound compute services (e.g., Playback Service), the scale target is set to **65% average CPU utilization**.
