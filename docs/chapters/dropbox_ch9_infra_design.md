...
```
       [ EKS Kubernetes Worker Node Array ]
         ...
         +-------------+-------------+
         v                           v
  [ Pod: Retrieval Service ]  [ Pod: Block Ingestion ]
...
kind: Deployment
metadata:
  name: retrieval-service-deployment
  namespace: dropbox-prod
spec:
  replicas: 48
  selector:
    matchLabels:
  ...
  template:
    ...
    spec:
      containers:
      - name: retrieval-service-container
        image: dropbox/retrieval-service:v3.2.0
        resources:
          requests:
            memory: "4Gi"
...
```
This configuration forces EKS to distribute `retrieval-service` pods across distinct availability zones, ensuring that an AZ outage does not disrupt service availability.
...
For CPU-bound compute services (e.g., File Retrieval Service), the scale target is set to **65% average CPU utilization**.
