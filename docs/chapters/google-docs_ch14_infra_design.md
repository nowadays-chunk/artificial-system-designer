# Google Docs Case Study - Chapter 14: Lifecycle Management
...
## 2. Upgrading MySQL Shards Without Downtime

To upgrade database engines with zero downtime, we use replication-based migrations:
1. **Target Cluster**: Deploy a new MySQL replica cluster running the target version.
...
```
    ...
    [ Source DB Cluster (v5.7) ] =======( Logical Replication )=======> [ Target DB Cluster (v8.0) ]
```
