...
```
       [ EKS Kubernetes Worker Node Array ]
         ...
         +-------------+-------------+
         v                           v
  [ Pod: Telemetry Gateway ]  [ Pod: Dispatch Matcher ]
  ...
  template:
    ...
    spec:
      ...
      - name: dispatcher-container
        image: uber/dispatcher-service:v3.2.0
        resources:
          requests:
            memory: "4Gi"
...
```
This configuration forces EKS to distribute `dispatcher-service` pods across distinct availability zones, ensuring that an AZ outage does not disrupt service availability.
...
For CPU-bound compute services (e.g., Dispatch Matcher), the scale target is set to **65% average CPU utilization**.
