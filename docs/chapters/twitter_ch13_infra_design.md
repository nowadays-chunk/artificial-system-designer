# Twitter Case Study - Chapter 13: Infrastructure Optimization
...
- **Cache Hit Ratio**: Home feed Redis cache hits must exceed 98%. Drops in this ratio shift read loads to the Cassandra database, increasing read latency.
- **Database Saturation**: Cassandra node CPU utilization and disk queue depth.
...
For Scala-based Timeline Servers, we use the **Z Garbage Collector (ZGC)**:
