# Dropbox Case Study - Chapter 11: Governance and Compliance
...
Global storage platforms must comply with data privacy regulations like **GDPR (General Data Protection Regulation)** and **CCPA (California Consumer Privacy Act)**. Key compliance requirements include:
...
2. **Cache Eviction**: The system evicts the user's active session and sync caches from Redis instantly.
3. **Asynchronous Hard Delete**: A background job processes the deletion log from a Kafka topic (`account-deletions`). It runs low-priority batch deletions against databases to remove all files, folders, and shared memberships associated with the user ID, avoiding database write saturation during peak hours.
...
export class DeletionPipeline {
  ...
  private pgClient = new PostgresClient({
    connectionString: "postgresql://prod-db:5432/dropbox"
  });
  ...
  private async executeHardDelete(userId: string) {
    ...
    // 1. Delete user's files and folder metadata from PostgreSQL
    const deleteFilesQuery = "DELETE FROM dropbox_metadata.files WHERE parent_folder_id IN (SELECT folder_id FROM dropbox_metadata.folders WHERE owner_id = $1)";
    await this.pgClient.query(deleteFilesQuery, [userId]);

    // 2. Delete user's shared folder memberships
    const deleteMembershipsQuery = "DELETE FROM dropbox_metadata.memberships WHERE user_id = $1";
    await this.pgClient.query(deleteMembershipsQuery, [userId]);
...
2. **Asynchronous Assembly**: A worker consumes the request and queries database systems (PostgreSQL) to retrieve the user's profile, files list, and shared directories.
3. **Packaging**: The worker compiles the dataset into a structured JSON payload, packages it into an encrypted ZIP file containing their binary files retrieved from block storage, and uploads it to a secure S3 bucket.
...
metadata:
  name: retrieval-service-pod
  labels:
    env: production
    owner: core-retrieval-team
    project: dropbox-platform
    cost-center: cc-3072
