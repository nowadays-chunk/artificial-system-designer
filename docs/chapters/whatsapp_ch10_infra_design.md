# WhatsApp Case Study - Chapter 10: Disaster Recovery and Business Continuity
...
```
         ...
         |  Message Ingest   |  < 2 minutes       |  < 1 second      |
         +-----------------------------------------------------------+
```
...
### 2.1 Media Storage Replication (Cross-Region Replication)
For media attachments and binary assets, we use multi-region storage replication:
...
For transactional user metadata and directory configurations, active-active setups can cause write conflicts. We deploy an **Active-Passive replication** model:
...
export class DNSFailoverController {
  private primaryRegionHealthUrl = "https://health.us-east.whatsapp.com";
  private standbyRegionHealthUrl = "https://health.us-west.whatsapp.com";
