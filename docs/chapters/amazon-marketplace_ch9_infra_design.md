...
```
       [ EKS Kubernetes Worker Node Array ]
         ...
         +-------------+-------------+
         v                           v
  [ Pod: Catalog Search ]     [ Pod: Checkout Service ]
  ...
  template:
    ...
    spec:
      ...
      - name: checkout-service-container
        image: amazon/checkout-service:v3.2.0
        resources:
          requests:
            memory: "4Gi"
...
```
This configuration forces EKS to distribute `checkout-service` pods across distinct availability zones, ensuring that an AZ outage does not disrupt service availability.
...
For CPU-bound compute services (e.g., Checkout Service), the scale target is set to **65% average CPU utilization**.
