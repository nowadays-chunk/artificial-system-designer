...
```
       [ EKS Kubernetes Worker Node Array ]
         ...
         +-------------+-------------+
         v                           v
  [ Pod: Timeline Server ]   [ Pod: Write Ingestor ]
...
kind: Deployment
metadata:
  name: timeline-server-deployment
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: timeline-server-container
        image: twitter/timeline-service:v2.1.0
        resources:
          requests:
            memory: "4Gi"
...
```
This configuration forces EKS to distribute `timeline-server` pods across distinct availability zones, ensuring that an AZ outage does not disrupt service availability.
...
For CPU-bound compute services (e.g., Timeline Server), the scale target is set to **65% average CPU utilization**.
...
- **Private Datacenters**: Host persistent storage tiers (Cassandra clusters, Redis caches, and Kafka brokers) on dedicated bare-metal nodes, avoiding public cloud network egress fees.
