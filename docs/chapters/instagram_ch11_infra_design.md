# Instagram Case Study - Chapter 11: Governance and Compliance
...
3. **Asynchronous Hard Delete**: A background job processes the deletion log from a Kafka topic (`account-deletions`). It runs low-priority batch deletions against databases to remove all posts, comments, likes, and followers associated with the user ID, avoiding database write saturation during peak hours.
...
export class DeletionPipeline {
  ...
  private pgClient = new PostgresClient({
    connectionString: "postgresql://prod-db:5432/instagram"
  });
  ...
  private async executeHardDelete(userId: string) {
    ...
    // 1. Delete user's posts from PostgreSQL
    const deletePostsQuery = "DELETE FROM instagram_metadata.posts WHERE author_id = $1";
    await this.pgClient.query(deletePostsQuery, [userId]);

    // 2. Delete user's social graph relations
    const deleteFollowersQuery = "DELETE FROM instagram_metadata.follows WHERE follower_id = $1 OR followed_id = $1";
    await this.pgClient.query(deleteFollowersQuery, [userId]);
...
2. **Asynchronous Assembly**: A worker consumes the request and queries database systems (PostgreSQL) to retrieve the user's profile, followers lists, and posts.
3. **Packaging**: The worker compiles the dataset into a structured JSON payload, packages it into an encrypted ZIP file along with their media binaries from S3, and uploads it to a secure S3 bucket.
...
metadata:
  name: feed-service-pod
  labels:
    env: production
    owner: core-feed-team
    project: instagram-platform
    cost-center: cc-1024
