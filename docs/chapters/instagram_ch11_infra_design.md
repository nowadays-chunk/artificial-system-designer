...
```
       [ Delete User Request ] ---> [ Soft Delete (Metadata flag) ]
         ...
         |                                                                             |
         v                                                                             v
  [ Delete SQL Rows (Comments, Likes) ]                                 [ Delete Objects (S3 Media) ]
```

- **Right to Be Forgotten**: When a user deletes their profile, the delete service deletes their record from PostgreSQL and publishes a deletion event to a Kafka topic. Background jobs consume the event to delete the user's photos from S3 and remove their associated comments and likes from DB tables.
- **Data Export**: Background workers package user data, comments, and S3 photos into encrypted ZIP archives, providing download links to users requesting data exports.

---

## 3. Resource Allocation Tagging

Every cloud resource (VM instance, database cluster, S3 bucket) must be tagged to track costs:
