# Amazon Marketplace Case Study - Chapter 11: Governance and Compliance
...
Global retail platforms must comply with data privacy regulations like **GDPR (General Data Protection Regulation)** and **CCPA (California Consumer Privacy Act)**. Key compliance requirements include:
...
2. **Cache Eviction**: The system evicts the user's active session and shopping cart caches from Redis instantly.
3. **Asynchronous Hard Delete**: A background job processes the deletion log from a Kafka topic (`account-deletions`). It runs low-priority batch deletions against databases to remove all orders, reviews, and search histories associated with the user ID, avoiding database write saturation during peak hours.
...
export class DeletionPipeline {
  ...
  private pgClient = new PostgresClient({
    connectionString: "postgresql://prod-db:5432/amazon"
  });
  ...
  private async executeHardDelete(userId: string) {
    ...
    // 1. Delete user's order logs from PostgreSQL
    const deleteOrdersQuery = "DELETE FROM amazon_metadata.orders WHERE customer_id = $1";
    await this.pgClient.query(deleteOrdersQuery, [userId]);

    // 2. Delete user's product reviews
    const deleteReviewsQuery = "DELETE FROM amazon_metadata.reviews WHERE author_id = $1";
    await this.pgClient.query(deleteReviewsQuery, [userId]);
...
2. **Asynchronous Assembly**: A worker consumes the request and queries database systems (PostgreSQL) to retrieve the user's profile, purchase orders, and payment histories.
...
metadata:
  ...
  labels:
    env: production
    owner: core-checkout-team
    project: amazon-platform
    cost-center: cc-7168
