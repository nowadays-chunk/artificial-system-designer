# Uber Case Study - Chapter 13: Infrastructure Optimization
...
- **Cache Hit Ratio**: Active driver locations and coordinate cache hits must exceed 98%. Drops in this ratio shift read loads to the PostgreSQL database, increasing read latency.
...
```
  ...
  [ Uncompressed Video ]     =======( Brotli Map Compression )=====> [ Stream Feeds (60% smaller) ]
```
...
### 2.2 Map Coordinate Compression
Geospatial coordinates and route maps are processed through compression pipelines:
- Route paths are compressed using **Brotli** or **ZSTD** algorithms before transmission, reducing network bandwidth footprints.
...
For Java-based Dispatch Matcher Services, we use the **Z Garbage Collector (ZGC)**:
