...
```
       [ EKS Kubernetes Worker Node Array ]
         ...
         +-------------+-------------+
         v                           v
  [ Pod: Feed Service ]      [ Pod: Media Ingestion ]
  ...
  template:
    ...
    spec:
      ...
      - name: feed-service-container
        image: instagram/feed-service:v3.2.0
        resources:
          requests:
            memory: "4Gi"
...
```
This configuration forces EKS to distribute `feed-service` pods across distinct availability zones, ensuring that an AZ outage does not disrupt service availability.
...
For CPU-bound compute services (e.g., Feed Service), the scale target is set to **65% average CPU utilization**.
