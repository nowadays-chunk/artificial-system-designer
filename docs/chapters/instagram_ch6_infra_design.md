...
While 0-RTT improves mobile connection startup latencies, it is vulnerable to **replay attacks**, where an eavesdropper intercepts a client's 0-RTT packet (e.g. a POST request to post a media file) and retransmits it to the server, potentially duplicating the action.
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const payload = jwt.verify(token, publicKey, {
        algorithms: ["RS256"],
        audience: "instagram-api-services"
      }) as any;
      ...
      if (req.method === "POST" && !req.context.scopes.includes("media:write")) {
