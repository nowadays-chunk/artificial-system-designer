# Dropbox Case Study - Chapter 12: Collaboration and Stakeholder Communication
...
- **Problem**: Users received HTTP 500 errors when loading directories or syncing files.
- **Why?**: The File Sync Service could not fetch cached metadata state from Redis.
- **Why?**: The Redis cluster node became unresponsive due to memory saturation.
- **Why?**: A large shared enterprise folder with 10,000 active members was modified, triggering a write-intensive fan-out storm that saturated the cache node.
- **Why?**: The hybrid pull-on-read model threshold was set to 50,000 members, which was too high to prevent write storms on large folders.
- **Remediation**: Adjust the folder membership write-bypass threshold to 500 members and enforce rate limits on fan-out queues.
