# YouTube Case Study - Chapter 11: Governance and Compliance
...
3. **Asynchronous Hard Delete**: A background job processes the deletion log from a Kafka topic (`account-deletions`). It runs low-priority batch deletions against databases to remove all videos, comments, and playlists associated with the user ID, avoiding database write saturation during peak hours.
...
export class DeletionPipeline {
  ...
  private pgClient = new PostgresClient({
    connectionString: "postgresql://prod-db:5432/youtube"
  });
  ...
  private async executeHardDelete(userId: string) {
    ...
    // 1. Delete user's videos and metadata from PostgreSQL
    const deleteVideosQuery = "DELETE FROM youtube_metadata.videos WHERE uploader_id = $1";
    await this.pgClient.query(deleteVideosQuery, [userId]);

    // 2. Delete user's comments
    const deleteCommentsQuery = "DELETE FROM youtube_metadata.comments WHERE user_id = $1";
    await this.pgClient.query(deleteCommentsQuery, [userId]);
...
2. **Asynchronous Assembly**: A worker consumes the request and queries database systems (PostgreSQL) to retrieve the user's profile, uploads list, and comment logs.
3. **Packaging**: The worker compiles the dataset into a structured JSON payload, packages it into an encrypted ZIP file containing their media files (retrieved from video buckets), and uploads it to a secure S3 bucket.
