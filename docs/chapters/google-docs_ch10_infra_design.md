...
```
         ...
         |  Edit Ingestion   |  < 2 minutes       |  < 1 second      |
         +-----------------------------------------------------------+
```
...
### 2.1 Media/Document Storage Replication (Cross-Region Replication)
For document files and binary assets, we use multi-region storage replication:
...
For transactional document metadata and stable snapshots, active-active setups can cause write conflicts. We deploy an **Active-Passive replication** model:
...
export class DNSFailoverController {
  private primaryRegionHealthUrl = "https://health.us-east.googledocs.com";
  private standbyRegionHealthUrl = "https://health.us-west.googledocs.com";
