# Netflix Case Study - Chapter 13: Infrastructure Optimization
...
- **Cache Hit Ratio**: Active video catalog and manifest cache hits must exceed 98%. Drops in this ratio shift read loads to the PostgreSQL database, increasing read latency.
...
Because the read-to-write ratio is asymmetric (200:1), data egress cost is a primary expense. We optimize egress transit through two strategies:

```
  Data Size Reduction:
  [ Raw API Payload (JSON) ] =======( Protobuf Serialization )=======> [ Binary Frames (40% smaller) ]
  [ Uncompressed Video ]     =======( AV1 / VP9 Video Compression )===> [ Stream Feeds (60% smaller) ]
```
...
### 2.2 Video Transcoding Optimization
Video payloads are processed through compression pipelines:
- Uploaded videos are encoded into **AV1** or **VP9** formats.
- AV1 provides equivalent video quality at a 30% lower bitrate than legacy VP9 profiles, reducing CDN bandwidth consumption.
...
For Java-based Playback Broker Services, we use the **Z Garbage Collector (ZGC)**:
