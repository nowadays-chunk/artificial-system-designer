# WhatsApp Case Study - Chapter 13: Infrastructure Optimization
...
- **Cache Hit Ratio**: Active user presence and session cache hits must exceed 98%. Drops in this ratio shift read loads to the PostgreSQL database, increasing read latency.
...
```
  ...
  [ Uncompressed Video ]     =======( Brotli Binary Compression )=====> [ Stream Feeds (60% smaller) ]
```
...
### 2.2 Binary Message Compacting
Message payloads are processed through compacting pipelines:
- Raw message blocks are compressed using **Brotli** or **ZSTD** algorithms before transmission, reducing network bandwidth footprints.
