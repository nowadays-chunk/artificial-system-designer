# Amazon Marketplace Case Study - Chapter 10: Disaster Recovery and Business Continuity
...
```
         ...
         |  Order Ingest     |  < 2 minutes       |  < 1 second      |
         +-----------------------------------------------------------+
```
...
### 2.1 Media/Product Image Storage Replication (Cross-Region Replication)
For product images and asset media files, we use multi-region storage replication:
...
For transactional order metadata and user configurations, active-active setups can cause write conflicts. We deploy an **Active-Passive replication** model:
...
export class DNSFailoverController {
  private primaryRegionHealthUrl = "https://health.us-east.amazon.com";
  private standbyRegionHealthUrl = "https://health.us-west.amazon.com";
