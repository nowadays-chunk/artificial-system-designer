# Netflix Case Study - Chapter 11: Governance and Compliance
...
Global streaming platforms must comply with data privacy regulations like **GDPR (General Data Protection Regulation)** and **CCPA (California Consumer Privacy Act)**. Key compliance requirements include:
...
2. **Cache Eviction**: The system evicts the user's active session and playback history caches from Redis instantly.
3. **Asynchronous Hard Delete**: A background job processes the deletion log from a Kafka topic (`account-deletions`). It runs low-priority batch deletions against databases to remove all videos watched data, billing credentials, and profiles associated with the user ID, avoiding database write saturation during peak hours.
...
export class DeletionPipeline {
  ...
  private pgClient = new PostgresClient({
    connectionString: "postgresql://prod-db:5432/netflix"
  });
  ...
  private async executeHardDelete(userId: string) {
    ...
    // 1. Delete user's profiles from PostgreSQL
    const deleteProfilesQuery = "DELETE FROM netflix_metadata.profiles WHERE user_id = $1";
    await this.pgClient.query(deleteProfilesQuery, [userId]);

    // 2. Delete user's billing records
    const deleteBillingQuery = "DELETE FROM netflix_metadata.billing WHERE user_id = $1";
    await this.pgClient.query(deleteBillingQuery, [userId]);
...
2. **Asynchronous Assembly**: A worker consumes the request and queries database systems (PostgreSQL) to retrieve the user's profile, watchlist, and billing histories.
...
metadata:
  name: broker-service-pod
  labels:
    env: production
    owner: core-broker-team
