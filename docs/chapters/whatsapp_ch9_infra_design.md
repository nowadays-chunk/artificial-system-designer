...
```
       [ EKS Kubernetes Worker Node Array ]
         ...
         +-------------+-------------+
         v                           v
  [ Pod: Presence Service ]   [ Pod: Ingestion API ]
...
kind: Deployment
metadata:
  name: presence-service-deployment
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: presence-service-container
        image: whatsapp/presence-service:v3.2.0
        resources:
          requests:
            memory: "4Gi"
...
```
This configuration forces EKS to distribute `presence-service` pods across distinct availability zones, ensuring that an AZ outage does not disrupt service availability.
...
For CPU-bound compute services (e.g., Presence Service), the scale target is set to **65% average CPU utilization**.
